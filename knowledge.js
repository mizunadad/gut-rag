...
const KNOWLEDGE = [
...

  {
    ...
    brainAreas: ['島皮質 Insula', '前頭前皮質'],
    mechanism: '島皮質は身体感覚・感情・認知を統合するハブ。「頭ではわかるが腑に落ちない」は、前頭前皮質（論理）と島皮質（身体統合）が一致していない状態。',
    abstract: '認知と身体感覚が一致した状態。Embodied Cognition（身体化された理解）の神経的実現。',
    coaching: 'コーチングで「わかりました」が「腑に落ちていない」時は、島皮質が未統合のサイン。問いを変えて身体反応を引き出す。',
...

  {
    ...
    mapId: 1,
    subtitle: 'Intuition',
    brainAreas: ['基底核', '小脳', '海馬'],
    mechanism: '反復学習により手続き記憶として自動化。意識を介さない高速処理（システム1）が洗練された状態。カーネマンの「速い思考」の精鋭版。',
    abstract: '意識の閾値より深い層での情報処理。暗黙知が神経回路にエンコードされた状態。',
    coaching: '「なんか違う」と感じたら、それは経験が無意識回路に落ちた証拠。言語化より先にある信号を信頼する訓練が専門家への道。',
...

  {
    ...
    layer: 'L2',
    mapId: 4,
    subtitle: 'Reading the Atmosphere',
    brainAreas: ['扁桃体', '上側頭溝 STS', '鏡像ニューロン系'],
    mechanism: '扁桃体は脅威や違和感を意識以前に検知（100ms以内）。上側頭溝は他者の動き・表情・視線の方向を統合処理する。',
    abstract: 'パターン認識が意識化される前に働く、専門家の暗黙知（ポランニー）の感知システム。',
    coaching: '「このサプライヤーなんか違う」は扁桃体が先に反応した信号。ベテランのこの感覚を若手に言語化させることが暗黙知の移転になる。',
...

  {
    ...
    layer: 'L1',
	  mapId: 3,
    subtitle: '腸は嘘をつかない',
    brainAreas: ['腸神経系 ENS', '迷走神経', '脳腸軸'],
    mechanism: '腸神経系は約1億のニューロンを持つ「第二の脳」。脳腸軸の信号は80〜90%が腸→脳方向。つまり腹の感覚が脳の判断を先導している。',
    abstract: '意識的思考は後付け正当化のバイアスを持つが、ENSはそれ以前の信号を出力している。',
    coaching: '品質現場の「なんか不良の匂いがする」は腸神経系が先に検知したシグナル。数値化されていなくても行動に値する。',
...

  {
    ...
    layer: 'L3',
    mapId: 8,
    subtitle: 'Inner Steadiness',
    brainAreas: ['前頭前皮質', '扁桃体制御回路', '前帯状皮質'],
    mechanism: '前頭前皮質が扁桃体の情動反応を抑制・調整する回路の強度。訓練と経験により扁桃体への下向き制御が強化される。',
    abstract: '外部環境の揺らぎに対して、内的基準軸が安定している状態。メタ認知と情動調整の統合。',
    coaching: '胆力は生まれつきではなく、前頭前皮質と扁桃体の接続が強化される過程。失敗体験を安全に処理できる環境が育てる。',
...

  {
    ...
    layer: 'L2',
    mapId: 6,
    subtitle: 'Reading the Interval',
    brainAreas: ['右半球', '小脳タイミング系', '基底核'],
    mechanism: '右半球は全体的・文脈的処理を担い、小脳は時間的精度（ミリ秒レベル）を調整する。非言語的リズムの同期がここで起きる。',
    abstract: '時間の位相差を感知する能力。言語の外側にある対話の文脈知覚。左脳では処理できない次元。',
    coaching: 'コーチング中の沈黙は「答えが生まれている時間」。それを埋めてしまう衝動は左脳優位のサイン。右脳を信頼して待つ。',
...

  {
    ...
    layer: 'L2',
    mapId: 5,
    subtitle: 'Far-sighted Intuition',
    brainAreas: ['デフォルトモードネットワーク', '海馬', '前頭前皮質'],
    mechanism: 'DMNは明示的情報のない状態で過去の全経験から予測・パターン補完を行う。いわば「経験のシミュレーター」。',
    abstract: '観測できない変数を、過去の全経験から補完する高次推論。意識的思考が停止した時に最も活発になる。',
    coaching: '「根拠は言えないけど」という発言こそDMNが動いている証拠。その感覚を棄却せず仮説として扱う姿勢を若手に伝える。',
...

  {
    ...
    layer: 'L3',
    mapId: 7,
    subtitle: 'Character / Being',
    brainAreas: ['内側前頭前皮質 mPFC', '側頭頭頂接合部 TPJ'],
    mechanism: 'mPFCは他者の内的状態・意図・人格を推定する「社会脳」の中枢。TPJは自己と他者の視点を切り替える。',
    abstract: '存在論的な文脈。ハイデガーの「現存在」──その人が世界とどう関わるかという存在様式。',
    coaching: '面接や初対面でmPFCが「信頼できる」と判定した時、それは統計的には正確。この感覚に理由をつける訓練が人を見る目を育てる。',
...

  {
    ...
    layer: 'L3',
    mapId: 9,
    subtitle: 'Virtue / Excellence',
    brainAreas: ['内側前頭前皮質 mPFC', '報酬系', '前帯状皮質'],
    mechanism: '自己一致した行動を取るとき、mPFCは「自己一致」シグナルを出し報酬系が活性化する。外的承認なしに「これでいい」と感じる状態。不一致時は前帯状皮質がエラー信号を出し続ける。',
    abstract: 'アリストテレスの「エウダイモニア」——善く生きることの核心。本来性（Authenticity）の神経的基盤。外から評価される「いい人」ではなく、本来の卓越性を発揮した状態。',
    coaching: '「いい人を演じている」感覚は、mPFCが自己不一致を検知している信号。自分の「アレテー」を問うことで、演じる必要がなくなる。',
    summary: '外から評価される「いい人」ではなく、本来の自分の卓越性を発揮した状態。',
    body: 'アレテー（ἀρετή）はギリシャ語で「卓越性・本来の機能を最大限に発揮した状態」を意味する。アリストテレスはこれを「エウダイモニア（善く生きること）」の核心と見なした。現代コーチング文脈では、他者期待ではなく自分本来の強みを発揮した状態として捉える。',
    etymology: 'アレテー（ἀρετή）= 卓越性・本来の機能を最大限に発揮した状態。',
    tags: ['アレテー', '本来性', '自己一致', 'mPFC', '徳'],
    socratic: ['hito', 'kokoro'],
    related: ['kotoba-007', 'tetsugaku-001', 'hannya-004'],
    plain_insight: '「いい人でいなければ」と感じるとき、本来の自分とズレが生じています。\nアレテー——あなただけが持つ卓越性——は、演じることと正反対の場所にある。\nアリストテレスは言った。善く生きることは、本来の機能を最大限に発揮することだと。\nその「本来」は、腸が知っている。',
    resonance_question: '今のあなたが「演じている」と感じる場面は、どこですか？',
    coaching_hint: {
      not_yet: '本来性に触れる前に、「演じている」ことへの気づきが必要。',
      ready: '「アレテー」を問うことで、評価軸が外から内へ移動する。'
    }
  },

  {
    ...
    layer: 'L3',
    mapId: 10,
    subtitle: 'Just-Sitting Certainty',
    brainAreas: ['デフォルトモードネットワーク', '島皮質', '前頭前皮質（静止）'],
    mechanism: '目標なき注意（mindfulness）でDMNが整理モードに入り、島皮質が現在の身体感覚を純粋に受信する。「考えない」時に浮上する確信。前頭前皮質の評価・比較機能が静止することで、深層の信号が表面に出る。',
    abstract: '西田幾多郎の「純粋経験」——主客未分の直接経験。評価・比較・目的を脱した存在そのもの。道元の「只管打坐」が示す、目的のない完全な現前性が、かえって最深の知を開く逆説。',
    coaching: '「なぜやるのか」を問い続けることで枯渇する局面がある。「ただやる」という只管打坐的姿勢が、かえって本質的な動機を再点火することがある。',
    summary: '根拠や目的を必要としない、純粋な現前性からくる確信。',
    body: '只管打坐（しかんたざ）とは道元禅師の言葉で「ただ座ること」。目的も評価もなく、ただ存在する完全な現前性を指す。西田幾多郎の「純粋経験」と通底する——主体と客体が未分化な直接経験。「なぜやるか」の問いが止んだとき、「ただやる」という確信が静かに浮上する。',
    etymology: '只管打坐（しかんたざ）= ただ座ること。道元禅師。目的のない完全な現前性。',
    tags: ['只管打坐', '純粋経験', 'DMN', '現前性', '道元'],
    socratic: ['kokoro', 'mono'],
    related: ['hannya-001', 'kotoba-007', 'tetsugaku-004'],
    plain_insight: '理由がわからないのに、確信がある——そういう瞬間はありませんか。\n「なぜ」を手放したとき、身体は答えを知っている。\n道元の「只管打坐」は、目的のない完全な現前性が、かえって最深の知を開くと説いた。\n腸はいつも、ただそこにある。理由なしに。',
    resonance_question: '根拠なく「これでいい」と感じた瞬間を、最近経験しましたか？',
    coaching_hint: {
      not_yet: '「なぜ」で詰まっているとき、「ただやる」という選択肢を示す。',
      ready: '只管打坐的姿勢が出たとき、それは深層からの確信の兆候。'
    }
  },
...

  {
    ...
    plain_insight: '262文字に、感覚と認識の間のすべてが書いてある。\n2500年前の人たちも、腸で感じることの大切さを知っていた。\n東の経典と西の神経科学が、独立に同じ問いにたどり着いていた。\n思想はいつも、体験の方が先に発見している。',
    resonance_question: '般若心経に触れたことがありますか？あるとすれば、そのとき体はどう感じましたか？',
    coaching_hint: null
  }
...
const EMOJI_KNOWLEDGE_MAP = {
  ...
  '自分自身':        ['kotoba-016', 'tetsugaku-002', 'hannya-001'],
  ...
  '空っぽ':          ['kotoba-017', 'hannya-001', 'hannya-003'],
  ...
  for (const id of order) {
    ...
    if (doc && doc.plain_insight && doc.resonance_question) {
      return {
        ...
        brainAreas:         doc.brainAreas || null,
        mechanism:          doc.mechanism || doc.body || null,
        abstract:           doc.abstract || null,
        coaching:           doc.coaching || doc.coaching_hint?.ready || null,
